...
bool LocalTx::initialize(void)
{
  ...
  */
  ...
  if (!cfg.getValue(name(), "VOICE_FILTER_BYPASS", voice_filter_bypass, true))
  {
    cerr << "*** ERROR: Illegal value for configuration variable "
         << name() << "/VOICE_FILTER_BYPASS\n";
    return false;
  }
  if (!voice_filter_bypass)
  {
    string ptt_type;
    if (cfg.getValue(name(), "PTT_TYPE", ptt_type, true) && (ptt_type == "NONE"))
    {
      voice_filter_bypass = true;
    }
  }
  ...
  {
    cout << name()
         << ": VOICE_FILTER_BYPASS enabled (or auto-enabled for PTT_TYPE=NONE)"
         << " - bypassing TX limiter, clipper and voiceband filter" << endl;
  }
  else
  {
