...
//  input_fifo->setOverwrite(true);
  ...
  prev_src = tone_dets;
    ...
    // for example CTCSS. Set VOICE_FILTER_BYPASS=1 to skip this for direct
    // PC microphone / VoIP use so the Opus codec receives full bandwidth.
  ...
  prev_src = clipper;
    ...
    // Skipped when VOICE_FILTER_BYPASS=1 (PC mic/VoIP) to pass full bandwidth.
  if (!voice_filter_bypass)
  {
#if (INTERNAL_SAMPLE_RATE == 16000)
