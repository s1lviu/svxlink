...
//  input_fifo->setOverwrite(true);
  ...
  prev_src = tone_dets;
    ...
    // for example CTCSS. This radio-specific processing can be bypassed for
    // direct PC microphone use by setting VOICE_FILTER_BYPASS=1, which allows
    // the full audio bandwidth supported by the Opus codec to pass through.
  bool voice_filter_bypass = false;
  cfg().getValue(name(), "VOICE_FILTER_BYPASS", voice_filter_bypass);
  if (!voice_filter_bypass)
  {
#if (INTERNAL_SAMPLE_RATE == 16000)
  ...
  prev_src = clipper;
    ...
    // Bypassed when VOICE_FILTER_BYPASS=1 to preserve the full bandwidth
    // that the Opus codec passes on to the network.
  if (!voice_filter_bypass)
  {
#if (INTERNAL_SAMPLE_RATE == 16000)
